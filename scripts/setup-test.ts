...
/**
 * Gmail Test Token Setup
 *
 * Generates OAuth token for Gmail server tests in local .tokens/ directory.
 *
 * Usage:
 *   npm run test:setup
 */
...
async function setupTest(): Promise<void> {
  ...
  console.log('🔐 Gmail Test Token Setup');
  console.log('');
  ...
  console.log(`   Token key: ${email}:gmail:token`);
  console.log('');
  console.log('Run `npm run test:unit` to verify Gmail API integration');
}
