...
if (typeof require === 'undefined') {
  ...
  eval(
    "import('../dist/esm/setup/version-help.js').then(({ handleVersionHelp }) => { const result = handleVersionHelp(process.argv.slice(2)); if (result.handled) { console.log(result.output); process.exit(0); } return import('../dist/esm/index.js').then((cli) => cli.default(process.argv.slice(2), 'mcp-gmail')); }).catch((err) => { console.error(err); process.exit(-1); });"
  );
} else {
  ...
  require('../dist/cjs/index.js')(process.argv.slice(2), 'mcp-gmail');
}
